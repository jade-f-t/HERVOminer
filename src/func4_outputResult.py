...
def get_padding(total_result, outputPath):
	...
	for peptide in total_result.keys():
		for region in total_result[peptide]:
			...
			bedCommand = f"bedtools getfasta -name -fi ./hg19/{total_result[peptide][region]['data']['chr']}.fa -bed {outputPath}/{peptide}_{region}.bed -fo {outputPath}/extracted_{peptide}_{region}_seq.fasta"
			try:
